...
cp -R "$ROOT_DIR"/css "$ROOT_DIR"/js "$ROOT_DIR"/photos "$ROOT_DIR"/videos "$DESTINATION/"
...
find "$DESTINATION" -maxdepth 1 -type f -name "*.html" -print0 | while IFS= read -r -d '' page; do
  sed -i \
    ...
    -e '/hx-get="https:\/\/api\.sethcharleston\.com\/test1/,/hx-trigger="load"/ s/hx-trigger="load"/hx-trigger="editor-disabled"/' \
